...
if (!defined('IKASKIN_FONT_SIZE'))         define('IKASKIN_FONT_SIZE',        ''); // Font size (ex. '93.75%' '15px')
...
if (!defined('IKASKIN_TABLE_WIDTH'))       define('IKASKIN_TABLE_WIDTH',      ''); // Width of table (ex. '100%' 'auto')
...
if (!defined('IKASKIN_IMAGE_BORDER'))      define('IKASKIN_IMAGE_BORDER',      0); // Border of image (0:None, 1:border, 2:shadow)
if (!defined('IKASKIN_LOGO'))              define('IKASKIN_LOGO',             'image/saitama.gif'); // Site logo image path (ex. 'image/pukiwiki.png') or SVG tag (ex. '<svg ...>...</svg>')
...
if (!defined('IKASKIN_CSS'))               define('IKASKIN_CSS',              'yoyastyle.css'); // Additional CSS file path (ex. 'mystyle.css')
...
if (!defined('IKASKIN_HEAD_CODE'))         define('IKASKIN_HEAD_CODE',        ''); // HTML code to insert in the head element (ex. Analytics snippet)
 ...
 * タイトル表示 by yoya
 */
$lines = get_source($title);
foreach ($lines as $line) {
    if (substr($line, 0, 6) == 'TITLE:') {
        $pageTitle = trim(substr($line, 6));
    }
}
...
if (IKASKIN_TABLE_WIDTH) $cssProperties .= '--table-width-user:' . IKASKIN_TABLE_WIDTH . ';';
if (IKASKIN_MENU_ORDER == 1) $cssProperties .= '--order-menubar:3;--order-rightbar:1;';
if (IKASKIN_IMAGE_BORDER) $cssProperties .= (IKASKIN_IMAGE_BORDER == 1)? '--image-border:1px solid var(--color-border);' : '--image-shadow:.1rem .1rem .3rem rgba(128,128,128,.333);';
...
<?php
	...
	//if ($enable_logout && !$havePageinfo) $tools .= (($tools)? '<span>|</span>' : '') . _navigator('logout');
...
if (PKWK_SKIN_SHOW_NAVBAR) {
	function __navigator($key, $value = '', $javascript = ''){
		$lang = & $GLOBALS['_LANG']['skin'];
		$link = & $GLOBALS['_LINK'];
		if (! isset($lang[$key])) { echo 'LANG NOT FOUND'; return false; }
		if (! isset($link[$key])) { echo 'LINK NOT FOUND'; return false; }
